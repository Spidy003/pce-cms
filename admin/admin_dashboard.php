...
<body>
    ...
    <header class="header-area">
        ...
        <a href="../auth/logout.php" class="neo-btn"
            style="background: var(--neo-pink); color: white; box-shadow: 5px 5px 0px black;">LOGOUT_</a>
    </header>
    ...
    <div class="hidden-data">
            ...
            <div class="admin-card">
                <form action="../core/process_admin.php?action=add_timetable" method="POST">
                    ...
                    <div style="display:grid; grid-template-columns: 2fr 1fr 1fr; gap:20px; margin-top:20px;">
                        <div><label>> SUBJECT_NAME</label><input type="text" name="subject" class="neo-input"
                                placeholder="e.g. Data Structures" required></div>
                        <div><label>> DAY</label>
                    ...
                    <button type="submit" class="neo-btn"
                        style="background:black; color:white; width:100%; margin-top:30px;">PUSH_TO_STUDENT_DASHBOARD</button>
                </form>
            </div>
        </div>
        ...
        <div id="tpl-class-mgmt">
            ...
            <div class="admin-card" style="border-color:var(--neo-green);">
                <form action="../core/process_admin.php?action=create_class" method="POST" style="display:flex; gap:10px;">
                    <input type="text" name="c_name" placeholder="New Class (e.g. IT-A)" class="neo-input"
                        ...
                        while ($s = $st->fetch_assoc()): ?>
                            <tr>
                                <td colspan="5" style="padding:0;">
                                    <form action="../core/process_admin.php?action=quick_enroll" method="POST"
                                        style="display:flex; width:100%; padding:15px; align-items:center;">
                                        <div style="flex:2;"><strong><?php echo $s['full_name']; ?></strong></div>
                                        <div style="flex:1;"><?php echo $s['admission_no'] ? $s['admission_no'] : 'N/A'; ?>
                                        </div>
                                        <div
                                            style="flex:1; font-family:'JetBrains Mono'; font-size:0.9rem; color:var(--neo-blue);">
                                            <strong><?php echo $s['class_name'] ? $s['class_name'] : 'NO_CLASS'; ?></strong>
                                        </div>
                                        <div style="flex:1;">
                                            <select name="batch" class="neo-input" style="padding:5px; margin:0;">
                                                <?php $b = $s['batch_name']; ?>
                                                <option value="B1" <?php if ($b == 'B1')
                                                    echo 'selected'; ?>>B1</option>
                                                <option value="B2" <?php if ($b == 'B2')
                                                    echo 'selected'; ?>>B2</option>
                                                <option value="B3" <?php if ($b == 'B3')
                                                    echo 'selected'; ?>>B3</option>
                                                <option value="ALL" <?php if ($b == 'ALL')
                                        ...
                                        <div style="flex:1; text-align:right;">
                                            ...
                                            <button type="submit" class="neo-btn"
                                                style="background:black; color:white; font-size:0.7rem;">
                    ...
                    <tbody>
                        ...
                        while ($u = $p_users->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><a href="process_admin.php?id=<?php echo $u['id']; ?>&action=approve" class="neo-btn"
                                        style="background:var(--neo-green);">GRANT</a></td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
        ...
        <div id="tpl-roles">
            ...
            <div class="admin-card">
                <form action="../core/process_admin.php?action=roleswap" method="POST">
                    ...
                    <button type="submit" class="neo-btn"
                        style="background:var(--neo-blue); color:white; width:100%; margin-top:20px;">EXECUTE_SWAP</button>
                </form>
            </div>
        </div>
        ...
        <div id="tpl-assign">
            ...
            <div class="admin-card">
                <form action="../core/process_admin.php?action=assign_subject" method="POST"
                    onsubmit="submitMapping(event, this)">
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                        <div>
                    ...
                    <button type="submit" class="neo-btn"
                        style="background:var(--neo-green); color:black; width:100%; margin-top:30px;">DEPLOY_MAPPING</button>
                    <div id="mapping-message"
                        style="margin-top: 15px; font-weight: bold; font-family: 'JetBrains Mono'; text-align: center;">
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="admin-card">
                <form action="../core/process_admin.php?action=csv_upload" method="POST" enctype="multipart/form-data">
                    <div style="border: 4px dashed black; padding: 40px; text-align: center; margin-bottom: 20px;">
                        ...
                        <p style="margin-top: 10px; font-family: 'JetBrains Mono';">Headers: name, email, password,
                            admission_no</p>
                    </div>
                    <input type="file" name="csv_file" class="neo-input" accept=".csv" required>

                    <button type="submit" class="neo-btn"
                        style="background:#ff00ff; color:white; width:100%; margin-top:20px;">COMMENCE_IMPORT</button>
                </form>
            </div>
        </div>
        ...
        <div id="tpl-broadcast">
            ...
            <div class="admin-card">
                <form action="../core/process_admin.php?action=post_notice" method="POST">
                    ...
                    <textarea name="content" class="neo-input" rows="5" placeholder="Enter details here..."
                        required></textarea>

                    <button type="submit" class="neo-btn"
                        style="background:black; color:white; width:100%; margin-top:20px;">TRANSMIT_TO_ALL</button>
                </form>
            </div>
        </div>
